...
| `cli_commands/handlers/binance.py` | `cli_commands/handlers/binance_live.py` owns LIVE guard/service construction, Spot LIVE and USDⓈ-M Futures LIVE status, balance, order-test, submit/cancel, and private-stream workflows | LIVE facade re-exports the historical handler names; both modules resolve dependencies through the CLI facade so safety guards and test seams remain centralized |
