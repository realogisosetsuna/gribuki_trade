...
| `services/live_trade_records.py` | `services/live_trade_records_parsing.py` owns OneBot private-event parsing and versioned propose/confirm/cancel command decoding | Parsing only constructs validated domain inputs; the record service retains allowlists, ledger writes, confirmation, and protection-task orchestration |
...
| `services/adversarial_macro.py` | `services/adversarial_macro_feature_flag.py` owns the BASELINE/SHADOW/ENFORCE progressive-release adapter and shadow observer delivery | The adapter only coordinates analyzer ports and in-memory tasks; provider calls and audit policy remain behind the existing service boundaries |
