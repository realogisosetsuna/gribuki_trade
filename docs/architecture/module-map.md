...
| `adapters/binance/gateway.py` | `adapters/binance/spot_order_list_parsing.py` owns OCO/OTO/OTOCO order-list and bulk-order snapshot decoding | Pure response decoding has no transport, credentials, order tracking, or reconciliation state; gateway retains signed requests and execution authority |
...
| `strategy_lab/discovery.py` | `strategy_lab/discovery_models.py` owns factor-template grammar, search budgets, candidate inventories, and redundancy-filter value objects | Models are research-only and broker-independent; the discovery facade retains candidate enumeration and redundancy correlation calculations |
| `services/binance/binance_shadow.py` | `services/binance/binance_shadow_models.py` owns shadow configuration, safety watermark, market quote, adapter snapshot, and session statistics | The model module has no stream or order side effects; the shadow facade retains validation, strategy decisions, paper execution, and recovery |
| `services/binance/binance_futures_unattended.py` | `services/binance/binance_futures_unattended_models.py` owns the immutable startup/reconciliation result | The model module is side-effect free; the Futures facade retains leases, reconciliation, private-stream processing, and order execution |
| `services/ashare/ashare_paper_day.py` | `services/ashare/ashare_paper_day_models.py` owns the immutable `PaperDayResult` | The model module has no scheduling or persistence side effects; the runner retains PAPER state transitions and durable notification workflow |
| `services/ashare/ashare_paper_day.py` | `services/ashare/ashare_paper_day_llm_manifest.py` owns analyzer identity, PIT evidence snapshot, preopen LLM context, and manifest compatibility contracts | Pure audit/manifest contracts have no network, SQLite, scheduler, or broker side effects; the runner retains session recovery, scheduling, persistence, and execution |
| `cli.py` | `cli_commands/handlers/ashare_review.py` owns candidate lifecycle and research-review command handlers | The handler has no execution authority; it resolves replaceable CLI runtime hooks lazily and the facade retains command dispatch and historical names |
| `cli.py` | `cli_commands/handlers/strategy_lab.py` owns research-only factor discovery and exit-policy evaluation handlers | The handler cannot promote or execute strategies; the CLI facade retains dispatch and compatibility exports |
| `gui/integrations.py` | `gui/integration_gateway.py` owns integration health value objects, gateway protocol, and credential/network operations | The gateway has no Qt widgets; `integrations.py` retains Qt state, background jobs, and panel wiring while re-exporting historical types |
| `ingest/search_discovery.py` | `ingest/search_providers.py` owns Tavily/SearXNG HTTP, JSON decoding, endpoint validation, and provider errors | Provider code has no SQLite, OMS, account, or event persistence side effects; the discovery facade retains multi-provider concurrency, failure backoff, clustering, and event generation |
| `adapters/binance/stream.py` | `adapters/binance/stream_models.py` owns public market event value objects and pure stream URL/name validation | Models and URL builders have no WebSocket connection state or credentials; the stream facade retains reconnect, sequence monotonicity, and socket lifecycle |
| `adapters/ashare/derivatives.py` | `adapters/ashare/derivatives_parsing.py` owns SSE official-document decoding, visibility checks, symbol/date validation, and pure option/ETF row projections | The parsing module has no HTTP client or retry state; the adapter facade retains requests, timeout/error translation, and public compatibility exports |
| `cli.py` | `cli_commands/handlers/ashare_paper_day.py` owns PAPER-day command validation and dispatch; `cli_commands/handlers/ashare_paper_day_run.py` owns resource setup, continuity checks, and runner lifecycle; `cli_commands/ashare_paper_day_results.py` owns sidecar-only status/report/summary projections | Handler modules use a lazy CLI facade to preserve historical monkeypatch paths; durable ledger, event, outbox, and notification transaction boundaries remain in the service runner; the CLI facade retains parser dispatch and compatibility exports |
| `cli.py` | `cli_commands/handlers/ashare_post_close.py` owns post-close command validation, the idempotent analysis/delivery state machine, report artifact verification, and NapCat durable-outbox delivery | The handler resolves replaceable CLI hooks lazily so existing monkeypatch paths remain valid; PAPER ledger, candidate, research, and notification stores retain their transaction ownership, and no LIVE authority is introduced |
| `cli.py` | `cli_commands/handlers/ashare_research.py` owns bounded research-watch cycles and dynamic watchlist/ACTIVE-candidate symbol resolution | The handler remains research-only and delegates one-symbol execution through a lazy CLI facade; it has no broker, account, order, or promotion authority |
| `cli.py` | `cli_commands/handlers/ashare_research.py` owns bounded A-share research-watch polling and watchlist/ACTIVE-candidate symbol resolution | The handler calls the facade's single-run research hook through a lazy boundary, preserving research-only behavior and monkeypatch paths; CLI dispatch and research persistence remain compatible facade responsibilities |
...
| `services/ashare/ashare_paper_day.py` | `services/ashare/ashare_paper_day_reasons.py` owns entry-rejection and single-minute match reason-code explanations and display fallbacks | Pure immutable reason mappings and text formatting; the runner facade retains historical constants/functions while ledger, event, scheduler, and execution side effects stay in the runner |
...
| `services/live/live_trade_orchestration.py` | `services/live/live_trade_orchestration_models.py` owns frozen protection inputs, input-provider protocol, work-run summaries, tracking observations, and stable input errors; `services/live_trade_orchestration.py` remains a compatibility facade | Models have no storage, notifier, or broker access; the orchestration service retains work claiming, exit lifecycle calls, and outbox delivery |
| `services/live/live_trade_records.py` | `services/live/live_trade_records_parsing.py` owns OneBot private-event parsing and versioned propose/confirm/cancel command decoding; `services/live_trade_records.py` remains a compatibility facade | Parsing only constructs validated domain inputs; the record service retains allowlists, ledger writes, confirmation, and protection-task orchestration |
| `services/exit/exit_plan_lifecycle.py` | `services/exit/exit_plan_lifecycle_models.py` owns lifecycle errors, append-only event-store protocol, QUICK/DEEP application results, and barrier observations; root `services/exit_plan_lifecycle*.py` paths remain compatibility facades | Contracts and results have no storage implementation or broker access; the lifecycle service retains event replay, idempotent append, and exit-plan orchestration |
| `services/macro/macro_research.py` | `services/macro/macro_evidence_selection.py` owns point-in-time evidence filtering, relevance ranking, injection rejection, publisher identity, and corroboration policy; `services/macro_research.py` remains a compatibility facade | Selection is pure apart from input event values; the macro facade retains request hashing, analyzer calls, and fail-closed execution |
...
| `reporting/paper_day_renderer.py` | `reporting/paper_day_rendering.py` owns pure audit sections, table formatting, and artifact-link projection helpers | Rendering helpers consume immutable projections only; the renderer facade retains complete report ordering, JSON event serialization, and Markdown contract validation |
...
| `services/ashare/ashare_paper_day.py` | `services/ashare/ashare_paper_day_reports.py` owns pure account-summary and daily-report projections from immutable snapshots/events | The projection module has no SQLite, network, scheduler, notifier, or broker access; the runner only reads the authoritative account/event stores, resolves exit plans, and keeps the historical facade methods |
...
| `services/macro/adversarial_macro.py` | `services/macro/adversarial_macro_serialization.py` owns canonical request, identity, analysis documents, hashes, and scalar normalization; `services/adversarial_macro.py` remains a compatibility facade | Pure audit encoding only; analyzer orchestration and provider calls stay in the service facade |
| `services/macro/adversarial_macro.py` | `services/macro/adversarial_macro_policy.py` owns role-output validation, peer envelopes, conservative aggregation, and round stability | Pure policy projection; analyzer facade retains provider calls, budgets, and shadow publication |
| `services/macro/adversarial_macro.py` | `services/macro/adversarial_macro_boundaries.py` owns role-request construction, failure-safe ABSTAIN analysis, and sanitized failure-call documents | Pure protocol boundary; analyzer facade retains concurrency, provider calls, budgets, and aggregation |
| `services/macro/adversarial_macro.py` | `services/macro/adversarial_macro_feature_flag.py` owns the BASELINE/SHADOW/ENFORCE progressive-release adapter and shadow observer delivery | The adapter only coordinates analyzer ports and in-memory tasks; provider calls and audit policy remain behind the existing service boundaries |
