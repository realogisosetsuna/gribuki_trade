...
`src/gribuki_trade/trading/futures_oms.py`. Verification evidence:
...
`tests/unit/test_futures_oms.py`, and the Spot Binance adapter tests under
`tests/unit/`.
...
| WebSocket `@bookTicker` | `BinanceBookTickerEvent` | Implemented | Best bid/ask only; update-id regression is rejected, but there is no gap/recovery state. |
...
| WebSocket `@depth` / `@depth@100ms` | `BinanceDepthEvent` and `depth_stream()` in `adapters/binance/stream.py` | **Partial (production blocker for order-book strategies)** | The transport parser now preserves `[U,u]`, optional `pu`, and absolute levels. The documented snapshot + buffered-diff state machine is still absent: on Spot the first event must bracket REST `lastUpdateId`; after that a gap requires a fresh snapshot. |
...
| OCO, OTO, OTOCO order lists | `submit_oco`, `submit_oto`, `submit_otoco` | Partial | Routes and typed requests exist. Native order-list identity and each leg must be persisted by the OMS; the advanced service currently remains a thin gateway wrapper. |
...
| Order-list query (`allOrderLists`, `openOrderLists`) and order-list status | `gateway.py` and `binance_spot_advanced.py` expose typed queries; `listStatus` is parsed | Partial | The routes and event are available, but Spot's durable OMS still needs an order-list table and atomic leg reconciliation before a strategy can rely on them after restart. |
...
`balanceUpdate`, and `listStatus`. A list event triggers a REST reconciliation
boundary in `BinanceSpotExecutionService`, so member legs are refreshed rather
than treating the list as one order. The Spot OMS still lacks a durable list
table and atomic leg projection, so unattended OCO/OTO/OTOCO recovery remains a
blocker.
...
| Futures local order-book recovery | Parser preserves `U/u/pu`; no local book state | Partial (production blocker) | Follow the official `/public/stream` + `/fapi/v1/depth` snapshot procedure; callers must verify the snapshot bracket before trusting the book. |
| 24-hour rotation, ping/pong, 10 msg/s, 1024 streams | `adapters/binance/futures_stream.py` | Partial | Routed `/public` and `/market` connectors, bounded jittered reconnect, rotation, sequence checks, and depth-gap fail-closed behavior are implemented. Snapshot application and a rate limiter remain above the adapter. |
...
and the public local-book snapshot component are wired above the transports.

## Readiness decision

The existing guarded paths are suitable for development and explicitly
confirmed LIVE probes. The repository is **not yet ready for unattended LIVE**
strategies that depend on a complete order book or Spot order-list recovery.
The minimum production-blocking implementation set is:

1. A shared depth-diff snapshot/recovery component for Spot and USDⓈ-M.
2. Wire the routed USDⓈ-M market connector to that snapshot component.
3. Add a durable Spot order-list table and atomic leg reconciliation.
4. Add account-scoped rate limiting/circuit breaking and a multi-hour network
   soak test for the private and public streams.
