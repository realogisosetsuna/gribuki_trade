...
| 政策/宏观 | PBOC、SAFE、NBS、财政部、发改委等官方发布 | P0-O 官方站点；P0-S 权威媒体转述只作抢先提示 | 以官方发布时间为准 | NBS/PBOC/Fed/CSRC/MOF/NDRC/SAFE/SSE 八源采集已实现；公告覆盖与持续可用性仍在验证 | 媒体抢发先标 `UNCONFIRMED`；官方原文到达后再进入模型评分 |
...
下表是**目标装配方案，不是当前调度器完成清单**。系统不应在 15:00 一次性等待所有数据，而应生成可追溯的多版本报告。每个版本记录 `report_as_of`、输入观测日、首次可见时间和缺失清单；后续版本追加/修订，不悄悄覆盖此前证据。当前仓库没有常驻应用调度器，表中的集合竞价、停牌、隔夜跳空与多时点补丁只有对应代码明确接入时才可视为已实现。
...
8. **PAPER-day 盘前降级种子**：只在上海当前自然日不晚于 09:25 使用；把当前网页快照谨慎绑定到已核验的上一交易日，所有行情/因子强制标记 `DEGRADED`，只承诺沪深主板、创业板和科创板并排除北交所。它不是历史收盘档案，且 PAPER 入场仍必须获得开盘后的当前时段 corroboration。
