"""A股 PAPER 日报的纯 Markdown 段落与审计区块渲染辅助。

本模块只接收已经完成的 sidecar 投影和不可变值对象，生成报告中的表格、
段落和审计行；不读取文件、不访问网络、不写入数据库，也不参与交易状态
转换。历史 ``paper_day_renderer`` 与 ``paper_day_summary`` 门面继续导出
相同的辅助对象身份。
"""
