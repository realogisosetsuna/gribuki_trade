...
def _llm_gate_document(outcome: IntradayLLMGateOutcome) -> dict[str, object]:
    return {
        ...
        "dual_track": _llm_gate_dual_document(outcome),
    }
...
def _llm_gate_dual_document(outcome: IntradayLLMGateOutcome) -> dict[str, object]:
    """投影两条独立语义轨道；结果缺失时明确降级而不补造结论。"""
...
def _llm_gate_dual_text(outcome: IntradayLLMGateOutcome) -> str:
    """把盘中双轨结果压缩成可在 QQ 中快速核对的一行中文。"""

    document = _llm_gate_dual_document(outcome)
...
def _deep_exit_sell_review(
    *,
    ...
    """让持久 DEEP 评分参与卖出紧迫度，但永不否决技术保护信号。"""
