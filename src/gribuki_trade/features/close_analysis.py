...
from gribuki_trade.features.close_analysis_models import (
    CloseAnalysisConfig,
...
from .close_analysis_indicators import (
    _amihud_bps_per_cny_billion,
    _amihud_illiquidity,  # noqa: F401 - historical private helper
    _amihud_illiquidity_with_coverage,
    _annualized_volatility,
    _average_true_range,
    _bollinger_bandwidth_series,
    _clamp_unit,
    ...
    _standard_deviation,
    _stochastic_k,
    _turnover_state,  # noqa: F401 - historical private helper
    _turnover_state_with_coverage,
    _wilder_adx,
)
...
def build_close_technical_assessment(
    ...
    *,
    ...
    discontinuity = _largest_previous_close_discontinuity(trading_bars[-resolved.minimum_history :])
    ...
    bollinger_bandwidth = (
        (bollinger_upper - bollinger_lower) / bollinger_mean if bollinger_mean > 0 else Decimal(0)
    )
    ...
    bollinger_bandwidth_state = (
        ...
        if prior_bollinger_bandwidths and _mean(prior_bollinger_bandwidths) > 0
        else Decimal(1)
    )
    bollinger_percent_b = (
    ...
    near_breakout = latest_close >= breakout_level * (Decimal(1) - resolved.near_breakout_fraction)
    ...
    compression_release = _sign(channel_position) * max(
        Decimal(0),
        _clamp_unit((bollinger_bandwidth_state - Decimal(1)) / Decimal("0.50")),
    )
    ...
    raw_directional_score = _clamp_score(sum((item.contribution for item in families), Decimal(0)))
    ...
    reduce_trigger = (
        breakdown or not trend_above_regime or (not close_above_trend and not short_above_trend)
    )
    entry_setup = (
        close_above_trend
    ...
    metrics = (
        ...
        *((f"family_score_{item.family_id}", item.score) for item in families),
    )
...
def _signal_family(
    ...
    resolved_status = status or (
        CloseSignalFamilyStatus.NEUTRAL if bounded == 0 else CloseSignalFamilyStatus.ACTIVE
    )
...
def _build_horizon_views(
    ...
    available = {family.family_id: family for family in families if family.weight > 0}
    definitions = (
        (
    ...
    for horizon, configured_weights, summary in definitions:
        configured_total = sum((weight for _, weight in configured_weights), Decimal(0))
        eligible = tuple(
            (family_id, weight)
        ...
        coverage = eligible_total / configured_total if configured_total > 0 else Decimal(0)
        views.append(
            CloseHorizonView(
                horizon=horizon,
                score=score,
...
def _audit_reasons(
    *,
    ...
    reasons.append(
        "POSITIVE_TWENTY_DAY_RETURN" if return_20d > 0 else "NON_POSITIVE_TWENTY_DAY_RETURN"
    )
    return reasons
